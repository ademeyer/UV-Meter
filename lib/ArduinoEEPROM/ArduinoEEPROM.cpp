...
ArduinoEEPROM::ArduinoEEPROM(uint8_t addr, uint16_t pg_size, uint32_t clock) 
...
int16_t ArduinoEEPROM::write_eeprom(uint16_t addr, uint8_t* data, uint16_t len)
{
    if(addr > 65535)
        addr = addr | B00000001;
...
int16_t ArduinoEEPROM::read_eeprom(uint16_t addr, uint8_t* data, uint16_t amt)
{
    int16_t _no = 0;
